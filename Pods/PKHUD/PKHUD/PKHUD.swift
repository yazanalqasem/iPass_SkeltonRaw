...
open class PKHUD: NSObject {
    ...
    open func show(onView view: UIView? = nil) {
        // Attempt to find a suitable view to present on.
        let viewToPresentOn: UIView = view ?? viewToPresentOn ?? currentSceneWindow() ?? UIApplication.shared.windows.first!
        
        // Ensure the container view is added to the view hierarchy and properly configured.
        if !viewToPresentOn.subviews.contains(container) {
            viewToPresentOn.addSubview(container)
            container.frame.origin = CGPoint.zero
            container.frame.size = viewToPresentOn.frame.size
    ...
    // Helper function to get the window of the current scene.
    private func currentSceneWindow() -> UIWindow? {
